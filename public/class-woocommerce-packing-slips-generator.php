 ...
 */
class WooCommerce_Packing_Slips_Generator extends WooCommerce_Packing_Slips {
	...
	public function setup_data($order_id)
	{
		...
		if(!$order) {
			wp_die( __('No Woo Order Found', 'woocommerce-packing-slips') );
		}
		...
		$order_meta_data = apply_filters('woocommerce_packing_slips_order_meta_data', $order_meta_data);
		...
		if(isset($order_data['customer_id']) && !empty($order_data['customer_id'])) {
			...
			$customer_meta_data = apply_filters('woocommerce_packing_slips_customer_meta_data', $customer_meta_data);
		...
		$this->data->order_data = apply_filters('woocommerce_packing_slips_order_data', $order_data);
		$this->data->items = apply_filters('woocommerce_packing_slips_order_items', $order->get_items());
    ...
    public function create_pdf($upload_dir, $output = false, $credit_note = false)
    {
    	...
    	try {
			...
			if($this->get_option('enableHeader')) {
				$header =  apply_filters('woocommerce_packing_slips_header', $this->get_header());
				$mpdf->SetHTMLHeader($header);
			}

			$html = '<div class="frame">';

				$html .= apply_filters('woocommerce_packing_slips_address', $this->get_address());
				$html .= apply_filters('woocommerce_packing_slips_content', $this->get_content());

			$html .= '</div>';

			if($this->get_option('enableFooter')) {
				$footer =  apply_filters('woocommerce_packing_slips_footer', $this->get_footer());
			...
			if($credit_note) {
				$html = str_replace( __('Invoice', 'woocommerce-packing-slips'), __('Credit Note', 'woocommerce-packing-slips'), $html);
			}
    ...
    public function get_address()
    {
    	...
    	$addressTextLeft = apply_filters('woocommerce_packing_slips_address_left', $this->get_option('addressTextLeft'));
    	$addressTextCenter = apply_filters('woocommerce_packing_slips_addresscenter', $this->get_option('addressTextCenter'));
    	$addressTextRight = apply_filters('woocommerce_packing_slips_address_right', $this->get_option('addressTextRight'));

		$address = '
		<div id="address-container" class="row">
			<div id="address-left" class="col col-4">' . wpautop( $addressTextLeft ) . '</div>
			<div id="address-center" class="col col-4">' . wpautop( $addressTextCenter ) . '</div>
			<div id="address-right" class="col col-4">' . wpautop( $addressTextRight ) . '</div>
		</div>';
    ...
    {
    	$contentTextIntro = apply_filters('woocommerce_packing_slips_content_intro', $this->get_option('contentTextIntro'));
    	$contentTextOutro = apply_filters('woocommerce_packing_slips_content_outro', $this->get_option('contentTextOutro'));
    ...
    public function get_items_table($items) 
    {
    	...
    	'<table class="content-items">
	    	<thead>
				<tr class="odd">
					' . ($showPos ? '<th width="10%">' . __('Pos.', 'woocommerce-packing-slips') . '</th>' : '') . '
					' . ($showProduct ? '<th width="30%">' . __('Product', 'woocommerce-packing-slips') . '</th>' : '') . '
					' . ($showSKU ? '<th width="20%">' . __('SKU', 'woocommerce-packing-slips') . '</th>' : '') . '
					' . ($showWeight ? '<th width="10%">' . __('Weight', 'woocommerce-packing-slips') . '</th>' : '') . '
					' . ($showQty ? '<th width="10%">' . __('Qty', 'woocommerce-packing-slips') . '</th>' : '') . '
				</tr>
	    	</thead>
	    	<tbody>';
    ...
    private function get_header_footer_type($type, $position)
    {
    	switch ($type) {
    		...
    		case 'bloginfo':
    			...
    			break;
    		case 'pagenumber':
				return __( 'Page:', 'woocommerce-packing-slips').' {PAGENO}';
    			break;
    		case 'productinfo':
